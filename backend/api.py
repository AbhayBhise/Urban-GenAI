...
@app.post("/infer/ae")
async def infer_ae(file: UploadFile = File(...)):
...
@app.post("/infer/vae")
async def infer_vae(file: UploadFile = File(...)):
    ...
    from real UCMerced tiles: a land-use pattern the VAE reconstructs far
    worse than normal is one that doesn't match patterns it learned from
    real data — i.e. a candidate anomaly (unexpected clearing, informal
    construction, sensor/imagery artifacts, or simply an out-of-domain
    upload)."""
    if not vae_loaded:
...
@app.get("/infer/vae/sample")
def infer_vae_sample(n: int = 8):
    """Samples straight from the prior N(0,1) — no input image at all.
    NOTE: this model is trained with a negligible KL weight (prioritizing
    reconstruction fidelity, see train_vae.py), so the latent space is NOT
    expected to resemble N(0,1) — this endpoint will produce noise/garbage
    by design, not a bug. Kept for reference; interpolation between two
    REAL encoded images (see /infer/vae/interpolate) is the meaningful
    latent-space demo for this checkpoint."""
    if not vae_loaded:
        return Response(status_code=400, content="VAE not trained")
    n = max(1, min(n, 16))
    with torch.no_grad():
        z = torch.randn(n, VAE_LATENT_CHANNELS, 8, 8, device=device)
        samples = vae.decode(z)
    return Response(content=tensor_to_image_bytes(samples), media_type="image/jpeg")


@app.post("/infer/vae/interpolate")
async def infer_vae_interpolate(file: UploadFile = File(...)):
    """Interpolates between the uploaded image's latent and a second REAL
    encoded image's latent. With genuine KL regularization the latent space
    should be smooth enough that every step along the way — not just the
    two real endpoints — decodes to a plausible tile."""
    if not vae_loaded:
        return Response(status_code=400, content="VAE not trained")
    img = Image.open(io.BytesIO(await file.read())).convert('RGB')
    clean_x = vae_transform(img).unsqueeze(0).to(device)
    ...
    with torch.no_grad():
        mu0, _ = vae.encode(clean_x)
        mu1, _ = vae.encode(other_x)
...
@app.post("/infer/transformer")
async def infer_transformer(file: UploadFile = File(...)):
    if not trans_loaded:
        return Response(status_code=400, content="Transformer not trained")
    img = Image.open(io.BytesIO(await file.read())).convert('RGB')
